...
from .usage import UsageEvent, UsageSummary, UsageTracker
...
def _budget_matches(budget: BudgetSpec, req: RunRequest) -> bool:
...
class Fleet:
        ...
        try:
            ...
            for b in self.budgets:
                if not _budget_matches(b, req):
                    continue
                try:
                    spent = _budget_spend_cached(cache, self.usage, self.session_start, b, now)
    ...
    def budget_status(self, now: datetime | None = None) -> list[BudgetStatus]:
        """Snapshot the configured budgets' windowed spend + remaining (limit - spend, floored at 0).

        Reuses the same windowed-spend logic as `_check_budget` so the warning threshold and the
        display stay in lockstep. The result is an empty list when no budgets are configured
        (preserves the "no behavior change" contract for users who don't opt in).
        """
