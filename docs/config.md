...
| Key | Type | Default | What it does | Example |
|-----|------|---------|--------------|---------|
| `backend` | string | *(required)* | Backend id to invoke (`opencode`, `cursor`, `codex`, `claude-code`, `antigravity`, `command-code`, `copilot`, `goose`, `zcode`, …). Goose models use `provider/model` (e.g. `cursor-agent/auto`) or a bare model; empty sides around `/` are rejected. | `backend: goose` |
| `model` | string \| omitted | `null` | Model id passed to the backend. OpenCode with no model defaults to `opencode-go/glm-5.2` at resolve time. OpenCode `fireworks-ai/*` models are allowed and warn (they bill Fireworks credits rather than the Go subscription, and report real per-run USD). Goose rejects malformed `provider/` / `/model` strings before spawn. | `model: claude-sonnet-4-6` |
...
| `MARSHAL_HOME` | path | `~/.marshal` | Where each repo's **run directories** live (`<MARSHAL_HOME>/worktrees/<repo>-<digest>/`) — and *only* that. It does **not** move the workspace registry (see `MARSHAL_WORKSPACES_FILE`, which defaults to `~/.marshal/workspaces.yaml` regardless of this) or any repo-local state under `<repo>/.marshal/` (ledger, run records, logs, reports). Set it to put run trees on another disk or to isolate a test run. **Do not point it inside a repo you run agents against:** run trees live outside the repo so a relative path from an agent's working directory cannot climb into your checkout or the ledger, and this is the one setting that gives that up. | `MARSHAL_HOME=/scratch/marshal` |
