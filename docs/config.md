...
| Key | Type | Default | What it does | Example |
|-----|------|---------|--------------|---------|
| `backend` | string | *(required)* | Backend id to invoke (`opencode`, `cursor`, `codex`, `claude-code`, `antigravity`, `command-code`, `goose`, `zcode`, …). Goose models use `provider/model` (e.g. `cursor-agent/auto`) or a bare model; empty sides around `/` are rejected. | `backend: goose` |
