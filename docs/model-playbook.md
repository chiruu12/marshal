...
| `antigravity` *(experimental)* | `gemini-3.1-pro-high` (heavy), `gemini-3.5-flash-low` (light), also `gemini-3.7-flash-*` / `claude-sonnet-4-6` / `claude-opus-4-6-thinking` / `gpt-oss-120b-medium` | varies | **unavailable** | probe `agy models` | **Model ids carry an effort suffix** — a bare family name is rejected (`--model gemini-3.5-flash` → `requires --effort`), and note `claude-sonnet-4-6` uses dashes, not dots. Worktree **writes** land correctly (worktree pre-registered as a trusted workspace). Supports `read-only` via `--mode plan` (verified: plans, writes nothing), so it can staff review teams for free — but plan mode *hard-denies* shell commands and exits non-zero, so prompt these reviewers to read files, not run commands; `safe-edit`/`yolo` share `--dangerously-skip-permissions`. Doctor verifies auth via print-mode `/usage`, which also reports weekly quota left. A goal starting with `/` is safe in write modes (`--disable-slash-commands`) but **refused up front in `read-only`**, where that flag would un-bind `--mode plan`. Needs `agy` ≥ 1.1.12. |
| `copilot` | `auto` (**the only id a Copilot Free account accepts**); paid plans pin from the CLI catalog, e.g. `claude-sonnet-5`, `gpt-5.6-luna`, `claude-haiku-4.5` | varies with `auto` | **unavailable** (output tokens only) | probe `copilot help config` → static `auto` + this table | GitHub Copilot CLI. `permission_fidelity=enforced-denies` — `--mode plan` genuinely refuses writes and `--deny-tool` beats `--allow-all-tools`. Free tier routes `auto` to models costing `premiumRequests: 0`. Pinning an id on a free plan fails fast with `Model "..." is not available`. Auth is a GitHub token (`COPILOT_GITHUB_TOKEN`/`GH_TOKEN`/`GITHUB_TOKEN`) or `copilot login`; doctor is path-only. |
...
- **unavailable** (`cursor`, `command-code`, `antigravity`, `zcode`, `copilot`, Goose when the provider reports no cost,
