...
| `supervisor_pid` | int \| null | Pid of the Marshal process supervising this run — the one that will write its outcome. A different question from `pid`, which names the agent subprocess: after the agent exits its supervisor still has pricing, a usage-API backfill, the `verify:` gate and artifact harvest to do, and the record reads `running` throughout. Startup reconciliation asks about the supervisor, so that a healthy mid-finalization run is not declared abandoned. `null` on records written before the field existed. |
