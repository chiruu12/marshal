...
| `message` | string | Detail on failure; base-branch drift warning when `base_branch_drift` is true. On `conflict`, reports when the run's base commit is reachable from **no branch or tag** — a base that is no longer in history makes every file read as changed on both sides, so the `conflicts` list points away from the real cause. It states that observation and offers the likely causes (history rewritten mid-run; a deleted base branch) rather than asserting one, since `base_branch` accepts any commit-ish. Silent unless the base is unreachable from the target **and** reached by no surviving ref, so a run deliberately spawned with `base_branch` onto a live branch is not mislabelled. |
