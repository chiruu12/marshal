...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `jobs` | list[Job] | *(required)* | Each job: `{ client?, goal, task_id?, context_files?, model?, backend?, duration?, workspace?, then? }`. Omit `client` and set `backend` for ad-hoc spawns. Per-job `workspace` overrides the call-level default. `then` uses the same field set (no nested `then` or `workspace`). |
