...
| `agent_survived_kill` | bool | `true` only on a `timed_out` run where Marshal signalled the agent's process group and then observed the leader **still alive**. Re-probed on every use, so it stops blocking once that process exits. The worktree is being written to, so its diff is not a snapshot: `commit_run`, `integrate` and `run_team` all refuse it while the pid stays live. `false` is the ordinary case, including a timeout whose kill landed — Marshal polls after signalling, so an ordinary timeout IS settled and remains reviewable. |
...
| `supervisor_start_time` | string \| null | OS-reported start time of `supervisor_pid`, making the pid an identity the way `pid_start_time` does for the agent, and rendered the same pinned way. Set together with `supervisor_pid` or not at all — a pid without a verifiable start time would be trusted on liveness alone. |
