...
| `list_models` | List the optional `models:` catalog (`id`, `backends`, `cost`, `quota_type`, `notes`) plus `driver_context`. With no catalog configured, `backend_models` maps each backend to `{models, source}` — `probed` (the CLI answered just now), `static` (a curated list that may be stale), or `unavailable`. A `static` list is **not** evidence the account can run those models. |
