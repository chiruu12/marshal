...
| `run_agent(client?, goal, task_id?, context_files?, read_paths?, base_branch?, model?, backend?, duration?)` | Run a task on a client's backend in an isolated worktree; returns the run record. Omit `client` for an ad-hoc spawn by `backend` (+ optional `model`). `duration` is a preset name or positive seconds. `base_branch` bases the worktree on a branch other than HEAD (e.g. after `commit_run`). `read_paths` is a read-only escape hatch for files outside the worktree (copied under `.marshal-context/` as immutable files/dirs; secret-shaped descendants and special files like FIFOs are refused). |
| `run_many(jobs, max_concurrency?)` | Run several `{client?, goal, task_id?, context_files?, read_paths?, workspace?, …}` jobs in parallel, each in its own worktree; per-job `workspace` allows mixed-repo batches under one concurrency cap. Returns all records (each tagged). |
| `spawn(client?, goal, task_id?, context_files?, read_paths?, base_branch?, model?, backend?, duration?)` | Start a run in the background; returns its RUNNING record at once - poll `get_run`/`status`. Same ad-hoc/`model`/`duration`/`base_branch`/`read_paths` rules as `run_agent`. |
