...
| `run_team(name, target, run_id?/base?/head?/pr?/paths?/text?)` | Run a panel of independent read-only reviewers over one subject (`run` diff, commit `range`, a `plan`, or an `audit` of the repo). **Reviewing a GitHub PR:** pass `pr=<number>` with `target='range'` — a pull request *is* a commit range, so there is no separate `pr` target and every `target: range` team reviews one unchanged. `pr` only resolves the endpoints (via `gh`, which must be on PATH) and diffs from the merge base, so the panel sees what the PR changed rather than what the base branch gained since it opened. Pass `pr` *instead of* `base`/`head`, not alongside. The reply echoes `pull_request` (title, URL, `stale` for an already-closed or merged PR) — confirm it names the PR you meant before reading the reviews. Returns `unified_report` (read first) plus each reviewer's full report; all persisted under `.marshal/reports/<stamp>-<team>-<id>/`. **Computes no verdict** — collecting the objections and deciding is the caller's job. Never integrates. Runnable form: [`examples/adversarial_review.py`](../examples/adversarial_review.py); team templates in [`examples/teams/`](../examples/teams/). |
