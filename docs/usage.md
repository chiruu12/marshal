...
| **backend** | A CLI adapter (cursor, opencode, codex, claude-code, command-code, copilot, goose, antigravity, zcode). Chosen per call, never global. |
...
- **`models`** (optional, top-level): a catalog the driver reads with `list_models` / `marshal models`.
  ...
  the configured backends' own catalogs under `backend_models`, each tagged with where it came
  from (`probed` / `static` / `unavailable`).
...
| Value | Meaning |
|-------|---------|
| `enforced-denies` | This tier installs a restriction beyond the worktree (curated denies, workspace sandbox, or plan/read-only mode). Not a true process sandbox. On backends: Cursor, OpenCode, Codex, Copilot safe-edit. |
...
| `list_models` | The model catalog: per model a `review`, a `weight` (heavy/standard/light) and `categories` (`best`, `cost-effective`, `fast`, `free`, `review-lens`) alongside the facts (`id`, `backends`, `cost`, `quota_type`, `notes`), plus `driver_context`. Weigh each review by its provenance: `evidence` is `measured` / `judgment` / `unverified`, `reviewed_on` is when it was last checked, and anything in `stale_reviews` is an opinion nobody has re-checked. `models_source` is `config` (this repo's own, tuned to its accounts), `shipped` (Marshal's default) or `none`. With no catalog configured, `backend_models` maps each backend to `{models, source}` — `probed` (the CLI answered just now), `static` (a curated list that may be stale, no probe attempted), `probe-failed` (a probe ran and failed; same curated list, unverified), or `unavailable`. A `static` list is **not** evidence the account can run those models. |
| `run_agent(client?, goal, task_id?, context_files?, read_paths?, artifacts_from?, base_branch?, model?, backend?, duration?)` | Delegate a goal to a worker agent in an isolated worktree; returns the run record. Product may be a diff or text (both first-class). Omit `client` for an ad-hoc spawn by `backend` (+ optional `model`). `duration` is a preset name or positive seconds. `base_branch` bases the worktree on a branch other than HEAD (e.g. after `commit_run`). `read_paths` is a read-only escape hatch for files outside the worktree (copied under `.marshal-context/` as immutable files/dirs; secret-shaped descendants, symlinks inside a declared tree, and special files like FIFOs are refused; copies open fail-closed). `artifacts_from` names earlier runs whose harvested artifacts this run may read (copied read-only under `.marshal-context/artifacts/<run_id>/`) — how a multi-round loop passes a report forward instead of pasting it into the next prompt. See [`examples/read_paths.py`](../examples/read_paths.py). |
| `run_many(jobs, max_concurrency?)` | Delegate several `{client?, goal, task_id?, context_files?, read_paths?, artifacts_from?, workspace?, then?, …}` jobs in parallel, each in its own worktree (each product may be a diff or text); optional per-job `then` runs a follow-up in the same worker as soon as that job's primary finishes (does not wait for sibling jobs). Per-job `workspace` allows mixed-repo batches under one concurrency cap. Returns one `{primary, then?, then_skipped?}` object per input job (input order): `primary` is the job's run; `then` is the follow-up when it ran; `then_skipped` explains why `then` did not (primary failed, no branch, primary's branch has no commits beyond its base, `commit_run` blocked, …). Pipelined review: [`examples/pipelined_review.py`](../examples/pipelined_review.py); mixed workspaces: [`examples/multi_workspace.py`](../examples/multi_workspace.py). |
| `spawn(client?, goal, task_id?, context_files?, read_paths?, artifacts_from?, base_branch?, model?, backend?, duration?)` | Start a worker agent in the background; returns its RUNNING record at once (before worktree provisioning finishes) - poll `get_run`/`status`; `cancel_run` works during setup. Same delegation primitive as `run_agent` (diff or text product). Same ad-hoc/`model`/`duration`/`base_branch`/`read_paths` rules. |
| `wait_for_runs(run_ids, timeout_s?)` | Block until those runs finish, or until `timeout_s` (default 60, max 600) — the close of the spawn loop, so a driver spends one tool call on a wait instead of a turn per poll tick. Ids may span workspaces; each is resolved once and they share one deadline. Returns `{settled, pending, unknown, timed_out, waited_ms}`. **`settled` means finished, not succeeded** — `failed`/`timed_out`/`cancelled`/`verify_failed` all land there, so branch on each record's `status` exactly as you would after a poll. Expiry is not an error: it returns the partial result with `timed_out: true`, so a client whose own request timeout is shorter degrades to a coarser poll rather than a broken call — re-call with the `pending` ids. `unknown` is ids with no record anywhere; they are reported at once and never waited on. Reports only: no implicit `collect_run`, no implicit `integrate`. A run whose supervisor died reads `running` forever and waits out the full timeout; `cancel_run` releases it. |
...
| `routing(task_kind?, window?)` | Which client's work actually got **kept**, per kind of task — your measured history, ranked, derived on read. Rates are over *judged* runs only, and the integration rate over *integrable* ones (`n_integrable` = judged minus `advisory`); each is `null` (unknown, not 0%) when its denominator is empty, and that denominator rides beside it. Cost never ranks a client it could not measure. Ranking is per `task_kind`, and `recommended` is `null` rather than a guess. |
| `set_outcome(run_id, outcome, note?)` | Record whether a finished run's work was any good: `rejected` (reviewed and wrong/off-scope), `abandoned` (given up on), or `advisory` (findings you used, nothing to merge — read-only reviews, audits, plan panels; kept out of the integration rate rather than counted against it). `integrated` is written for you by `integrate` and is **sticky** — overwriting it returns `conflict`, not an error. **Record your rejections:** declining to integrate leaves no trace, so an unjudged run and one you threw away look identical, and every `routing` rate is computed over judged runs only, and the integration rate over the judged runs that could have merged. Status ∈ `recorded` / `unchanged` / `conflict`. |
...
marshal models             # the model catalog: review, weight, categories, review freshness (--category X, --stale, --json)
...
Antigravity (`-p /usage`, print-mode) also verifies auth; ZCode and Copilot stay path-only (no cheap auth probe). Doctor is preflight only — it does not hard-block
...
| Antigravity | yes | no | `permission_fidelity=boundary-only`. Worktree writes work (the run's worktree is pre-registered in trustedWorkspaces and passed via `--add-dir`); supports `safe-edit`/`yolo`, and `read-only` maps to `--mode plan` (which hard-denies shell commands, so prompt those reviewers to read files rather than run them). Doctor verifies auth via a print-mode `/usage` probe and fails closed when it cannot. PTY wrapper still TODO. **Single Marshal process per host** when using this backend — see note below. |
...
| Copilot CLI | yes | tokens only (output) | `permission_fidelity=enforced-denies`. Runs `copilot -p` with `--output-format json` (JSONL). Tiers: `--mode plan` (genuinely refuses writes), `--allow-all-tools` + a curated `--deny-tool` overlay (`shell(rm)`, `shell(git push)`, `shell(gh:*)`, `write(.env*)`, `write(.git)`) for safe-edit, `--allow-all` for yolo. Every run also passes `--disable-builtin-mcps` (the built-in GitHub MCP acts on the real repo with your token, outside the worktree), `--no-remote`, and `--no-auto-update`. **Model pinning is plan-gated:** a Copilot **Free** account rejects every pinned id and accepts only `model: auto` (which routes to `premiumRequests: 0` models); paid plans can pin from `marshal models copilot`. Reports output tokens but **no cost**, so usage stays `unavailable`. Auth is a GitHub token (`COPILOT_GITHUB_TOKEN` / `GH_TOKEN` / `GITHUB_TOKEN`, or `copilot login`); doctor is path-only. |
