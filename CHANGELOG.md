...
- **EastRouter reconciliation could bill a concurrent run's charge here as measured cost.** The
  ...
  tokens than the run reported is the propagation-delay case it exists for, while matching *more*
  means prompt tokens this run never sent. Over-counting is allowed
  only as far as the run's own cache reads explain it - `input_tokens` excludes those while a
  provider may bill cached tokens inside `prompt` - and refused beyond; a backend that nests cache
  reads inside `input_tokens` reports none and so earns no allowance. Cache-heavy runs that
  previously fell back to `unavailable` under the symmetric rule now attribute correctly. The
  short side is unchanged and still has a gap this does not close: when a run's own record is the
  one still propagating, a lone foreign record can match short and be attributed in full. Settling
  that needs a request id, which Codex does not surface.
...
- **`SECURITY.md` now documents how to contain a run at the OS level, verified end to end.**
  ...
  narrowest verified profile, the write paths that are easy to miss - the git common dir when the repo is itself a
